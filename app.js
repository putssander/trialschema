...
async function previewAndDetectFormat(f) {
  ...
  try {
    ...
    if (ext === "jsonl") {
      ...
      setFormatBanner("ok", `Detected <strong>${state.format === "trialgpt" ? "TrialGPT JSONL" : "Org-specific JSONL"}</strong> &middot; ${lines.length} rows.`);
      return;
    }
    if (ext === "json") {
      const text = await f.text();
      ...
      const arr = Array.isArray(parsed) ? parsed : (parsed.trials || parsed.data || []);
      if (!arr.length) { setFormatBanner("warn", "No trial rows found in JSON."); return; }
      state.format = detectFormat(f.name, arr[0]);
      setFormatBanner("ok", `Detected <strong>${state.format === "trialgpt" ? "TrialGPT JSON" : "Org-specific JSON"}</strong> &middot; ${arr.length} rows.`);
      return;
    }
...
function parseTextByExt(name, text) {
  ...
  if (ext === "json") {
    try {
      ...
      const arr = Array.isArray(j) ? j : (j.trials || j.data || []);
...
where:
  ...
  - standard_code: OPTIONAL. Your best-guess ontology code (LOINC, SNOMED-CT, or ICD-10)
                   for the metric — e.g. "LOINC:4548-4" for HbA1c, "LOINC:2160-0" for creatinine,
                   "SNOMED:271649006" for systolic BP. Use "" when unsure. The downstream
                   matching engine uses this to align with patient EHR fields.
  - operator: one of  "<", "<=", ">", ">=", "=", "!=", "between".
  - value: numeric. For "between" use the lower bound and add upper_value.
  - unit: explicit unit string (e.g. "%", "pg/mL", "years", "ml/min", "mmHg"). Use "" when unitless.
...
Always emit at least one phase in care_path.phases (infer from drugs / disease / activities).
...
function sanitizeTrial(t) {
  ...
  // DAG funnel order: cheapest deterministic checks first. Sort by priority,
  // then put inclusions before exclusions, then fall back to original index.
  t.criteria.sort((a, b) => {
...
function sanitizeCriterion(c, i) {
  ...
  // Priority is the DAG funnel tier (1\u20135). Clamp out-of-range values.
  ...
  const out = {
    ...
    category,
    priority_level: prio,
    status: ["active", "inactive", "limited_to_documents"].includes(c.status) ? c.status : "active",
    routing: {
...
function updateRow(root, it) {
  ...
  const counts = it.result?.criteria
    ? `${it.result.criteria.length} criteria · ${it.result.care_path?.phases?.length || 0} phases`
    : "";
  ...
  host.innerHTML = manualPanel + `
    ...
    <div class="mt-4 bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200">
        <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500">Criteria \u2014 drag rows to set priority</h3>
      ...
      <div class="px-4 py-2 border-b border-slate-100 flex items-center gap-3 text-[10px] text-slate-500">
        <span class="font-semibold uppercase tracking-wider">Doc routing legend:</span>
        <span class="inline-flex items-center gap-1"><span class="inline-block w-3 h-3 rounded bg-blue-600"></span> Primary (required)</span>
        <span class="inline-flex items-center gap-1"><span class="inline-block w-3 h-3 rounded border border-amber-400 bg-amber-100"></span> Fallback (optional)</span>
        <span class="inline-flex items-center gap-1"><span class="inline-block w-3 h-3 rounded border border-slate-200 bg-white"></span> Off</span>
        <span class="ml-auto italic text-slate-400">Click a cell to cycle: off \u2192 primary \u2192 fallback \u2192 off</span>
      </div>
        ...
        <table class="w-full text-[12px]">
          <thead class="bg-slate-50 text-slate-500">
            <tr class="text-left align-bottom">
              ...
              <th class="w-20 px-2 py-2">Type</th>
              <th class="px-2 py-2">Criterion</th>
              <th class="w-40 px-2 py-2">Evaluation</th>
              <th class="w-24 px-2 py-2">Status</th>
              ${MATRIX_DOCS.map(d => `<th class="px-1 py-2 text-center w-12" title="${escapeHtml(d.label)} \u2014 ${escapeHtml(d.hint)}"><div class="text-[10px] font-semibold text-slate-600">${escapeHtml(d.short)}</div><div class="text-[9px] font-normal text-slate-400">${escapeHtml(d.group.replace("_layer","").replace("_core",""))}</div></th>`).join("")}
              <th class="px-1 py-2 text-center w-48" title="Free-text guidance for criteria whose category does not map to a structured document domain (consent, willingness, logistics, \u2026)"><div class="text-[10px] font-semibold text-orange-500">Other</div><div class="text-[9px] font-normal text-slate-400">guidance</div></th>
            </tr>
          </thead>
...
// Status cycle order (used by the single-button status column).
const STATUS_CYCLE = [
  { id: "active",               label: "Active",    cls: "bg-emerald-600 text-white border-emerald-600" },
  { id: "limited_to_documents", label: "Docs only", cls: "bg-blue-600 text-white border-blue-600" },
  { id: "inactive",             label: "Inactive",  cls: "bg-slate-600 text-white border-slate-600" },
];
...
function buildCriterionRow(trial, c, ci) {
  ...
  tr.draggable = true;
  tr.dataset.cid = c.criterion_id;
  const typeColor = c.type === "exclusion"
    ? "bg-rose-50 text-rose-700 border-rose-200"
    : "bg-emerald-50 text-emerald-700 border-emerald-200";

  // Quantitative pill or boolean tag
  const evalCell = c.evaluation_type === "quantitative" && c.structured_target
    ? `<span class="badge bg-indigo-50 text-indigo-700 border border-indigo-200">${escapeHtml(c.structured_target.metric)} ${escapeHtml(c.structured_target.operator)} ${c.structured_target.value}${c.structured_target.upper_value!==undefined?`\u2013${c.structured_target.upper_value}`:""} ${escapeHtml(c.structured_target.unit||"")}</span>${c.structured_target.standard_code ? `<div class="mt-1 text-[10px] font-mono text-slate-400" title="Ontology code suggested by the LLM \u2014 used by the downstream matching engine to align with patient EHR fields">${escapeHtml(c.structured_target.standard_code)}</div>` : ""}`
    : `<span class="badge bg-slate-100 text-slate-600">boolean</span>`;

  // Matrix cells (one <td> per doc, button cycles primary/fallback/off).
  const matrixCells = MATRIX_DOCS.map(d =>
    `<td class="px-1 py-2 text-center" data-doc="${d.id}"></td>`
  ).join("");

  // Stable id used by the guidance datalist below.
  const guidanceListId = `guidance-list-${trial.trial_id}-${c.criterion_id}`.replace(/[^a-z0-9_-]/gi, "_");
  const hasGuidance = !!(c.guidance && c.guidance.trim());

  // A criterion's category is "other" when the LLM-assigned label does not
  // match any known document-hierarchy entry. Guidance is only meaningful (and
  // shown) for "other" \u2014 matching categories already imply a routing rule.
  ...
  const categoryLabel = isOtherCategory
    ? "other"
    : (DOC_BY_ID[categoryNorm]?.short || DOC_BY_ID[categoryNorm]?.label || categoryNorm);
  const showGuidance = isOtherCategory;
  // DAG tier badge colour (cheapest \u2192 hardest).
  const tierColour = ({
  ...
  })[c.priority_level] || "bg-slate-50 text-slate-500 border-slate-200";
  const tierTitle = ({
    1: "Tier 1 \u2014 Structured EHR knock-out (no LLM needed)",
    2: "Tier 2 \u2014 High-yield unstructured (single-doc LLM extraction)",
    3: "Tier 3 \u2014 Complex timeline / cross-document",
    4: "Tier 4 \u2014 Other / soft criteria",
    5: "Tier 5 \u2014 Unranked",
  })[c.priority_level] || "DAG funnel tier";

  tr.innerHTML = `
    <td class="px-2 py-3 text-slate-300 cursor-grab select-none" data-handle title="Drag to reorder priority">
    ...
    <td class="px-2 py-3" data-prio><span class="badge border ${tierColour}" title="${escapeHtml(tierTitle)}">T${c.priority_level}</span></td>
    <td class="px-2 py-3"><span class="badge border ${typeColor}">${escapeHtml(c.criterion_id)}</span></td>
    <td class="px-2 py-3 text-slate-800">
      <div class="text-[12px] leading-snug">${escapeHtml(c.original_text)}</div>
      <div class="mt-1 flex items-center gap-2 text-[10px] uppercase tracking-wider text-slate-400">
        <span class="${isOtherCategory ? "text-orange-400 font-semibold" : ""}">${escapeHtml(categoryLabel)}</span>
      </div>
    </td>
    <td class="px-2 py-3">${evalCell}</td>
    <td class="px-2 py-3" data-status></td>
    ${matrixCells}
    <td class="px-1 py-2 align-top text-center" data-other-cell>
      ${showGuidance ? `
        <button type="button" data-other-toggle class="w-7 h-7 rounded-md border text-[10px] font-bold transition mx-auto flex items-center justify-center"></button>
        <div data-guidance-wrap class="mt-1 ${c.other_active ? "" : "hidden"}">
          <textarea data-guidance list="${guidanceListId}" rows="2"
            placeholder="Guidance for this \u2018other\u2019 criterion\u2026"
            class="w-full text-[11px] leading-snug rounded-md border border-amber-200 bg-amber-50/40 focus:bg-white focus:border-amber-400 focus:outline-none px-2 py-1 resize-y normal-case tracking-normal text-left">${escapeHtml(c.guidance||"")}</textarea>
  ...
  // Inline stage-expansion control: detected by regex (FIGO IB2-IVA, AJCC III,
  // T1-T3 N0-N1, etc.). Clicking calls the LLM to enumerate the explicit set
  // of stages so downstream document checks don't rely on implicit knowledge.
  ...
  if (stageMatch) {
    ...
    if (expanded.length) {
      stageBar.innerHTML = `<span class="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">${escapeHtml(stageMatch.system)}:</span>` +
        expanded.map(v => `<span class="badge bg-violet-50 text-violet-700 border border-violet-200">${escapeHtml(v.stage)}${v.tnm ? ` <span class="text-violet-400 font-normal">${escapeHtml(v.tnm)}</span>` : ""}</span>`).join("");
    } else {
      ...
      btn.title = "Use the LLM to enumerate explicit stages with TNM-8 mappings (no implicit knowledge required at runtime).";
      btn.addEventListener("click", () => expandStagesForCriterion(c, stageMatch, stageBar));
      stageBar.appendChild(btn);
    }
    textCell.appendChild(stageBar);
  }

  // Status: single cycling button.
  const statusHost = tr.querySelector("[data-status]");
  const renderStatus = () => {
    const cur = STATUS_CYCLE.find(s => s.id === c.status) || STATUS_CYCLE[0];
    statusHost.innerHTML = "";
    const b = document.createElement("button");
    b.className = `w-full text-[10px] font-semibold rounded-md px-2 py-1 border ${cur.cls}`;
    b.textContent = cur.label;
    b.title = "Click to cycle: Active \u2192 Docs only \u2192 Inactive";
    b.addEventListener("click", () => {
      const idx = STATUS_CYCLE.findIndex(s => s.id === c.status);
      c.status = STATUS_CYCLE[(idx + 1) % STATUS_CYCLE.length].id;
      renderStatus();
    });
  ...
  // Matrix: each cell cycles off \u2192 primary \u2192 fallback \u2192 off.
  MATRIX_DOCS.forEach(d => {
    const cell = tr.querySelector(`[data-doc="${d.id}"]`);
    renderMatrixCell(cell, c, d);
  });

  // "Other" trailing cell: behaves like a matrix checkbox \u2014 when toggled on,
  // a guidance textarea is revealed beneath it. Only rendered for criteria
  // whose category is "other".
  const guidance     = tr.querySelector("[data-guidance]");
  const dlist        = tr.querySelector("[data-guidance-list]");
  const otherToggle  = tr.querySelector("[data-other-toggle]");
  const guidanceWrap = tr.querySelector("[data-guidance-wrap]");
  if (otherToggle) {
    const paintToggle = () => {
      const on = !!c.other_active;
      otherToggle.title = `Other guidance \u2014 ${on ? "on" : "off"}`;
      otherToggle.className = `w-7 h-7 rounded-md border text-[10px] font-bold transition mx-auto flex items-center justify-center ${
        on
          ? "bg-amber-500 text-white border-amber-500 hover:bg-amber-600"
          : "bg-white text-slate-300 border-slate-200 hover:border-amber-300 hover:text-amber-500"
      }`;
      otherToggle.textContent = on ? "\u2713" : "";
    };
    paintToggle();
    otherToggle.addEventListener("click", () => {
      c.other_active = !c.other_active;
      paintToggle();
      guidanceWrap.classList.toggle("hidden", !c.other_active);
      if (c.other_active && guidance) { refreshGuidanceSuggestions(dlist, c); guidance.focus(); }
    });
  }
  if (guidance) {
...
function renderMatrixCell(cell, criterion, doc) {
  cell.innerHTML = "";
  const state = docRoutingState(criterion, doc.id);
  const btn = document.createElement("button");
  btn.title = `${doc.label} \u2014 ${state}`;
  const base = "w-7 h-7 rounded-md border text-[10px] font-bold transition mx-auto flex items-center justify-center";
  if (state === "primary") {
    ...
    btn.className = `${base} bg-white text-slate-300 border-slate-200 hover:border-blue-300 hover:text-blue-500`;
    btn.textContent = "";
  }
  btn.addEventListener("click", () => {
    const cycle = { off: "primary", primary: "fallback", fallback: "off" };
    setDocRouting(criterion, doc.id, cycle[state]);
    renderMatrixCell(cell, criterion, doc);
  });
  cell.appendChild(btn);
}

// HTML5 drag-and-drop reordering. After every drop we re-emit
// priority_level = 1..N based on the new visual row order.
function enableCriterionDrag(tbody, trial) {
  let dragSrc = null;
  tbody.querySelectorAll("tr").forEach(tr => {
    tr.addEventListener("dragstart", (e) => {
      ...
      e.dataTransfer.setData("text/plain", tr.dataset.cid || "");
    });
    tr.addEventListener("dragend", () => tr.classList.remove("opacity-50"));
    ...
    tr.addEventListener("drop", (e) => {
      e.preventDefault();
      // Reorder the criteria array to match the new DOM order. The
      // priority_level (DAG tier) is NOT rewritten by drag \u2014 the tier is set
      // by the LLM at extraction time. Manual drag is for fine-tuning the
      // execution order within a tier.
      const order = Array.from(tbody.querySelectorAll("tr")).map(r => r.dataset.cid);
...
function exportUnified() {
  ...
  const out = {
    schema: "trialschema/v1",
    generated_at: new Date().toISOString(),
    trial_count: Object.keys(merged).length,
    trials: Object.values(merged),
  };
